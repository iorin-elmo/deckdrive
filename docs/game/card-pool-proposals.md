...
山札検索や公開によって初めて選択候補が分かる効果は、まだ見ていないIDを最初の `PLAY_CARD.choices` に要求してはならない。効果解決中に非公開の `CARD_CHOICE_REQUESTED` を所有者へ送り、`choiceRequestId` と所有者に見える候補IDを提示して解決を一時停止する。battle state/replayには元のカード解決、残りの解決手順、候補IDを含む保留選択を保存する。他のプレイヤーのactionやターン進行はこの選択が終わるまで受け付けない。所有者は `SUBMIT_CARD_CHOICE` action でrequest IDと提示候補のいずれかを返し、サーバーは未解決request・所有者・候補内IDを検証して同じカード解決を再開する。複数の未知選択が続く場合は一つずつ要求する。choice requestと提出選択のID・候補情報は権威replayに記録し、プレイヤー向け投影では相手に伏せる。`矢継ぎ`、`変成液`、`戦術の確認` はこの継続選択を使う。`賢者の触媒` が次の合成前に素材を山札から手札へ加えた場合は、その素材も候補に含める後続の素材選択にこの継続選択を使う。任意対象がない場合は各カードに記載したとおり対象依存効果だけを行わず、残りの効果は解決する。choices はサーバー権威のaction/replayに完全な値を保存し、プレイヤー向けaction/replayでは各選択の可視性に従って相手に非公開の山札カードIDを伏せる。イベントと状態変更は同じ完全な選択から再生する。
