...
ドローを一つの演出に合成できるのは、同じ `playerId` の `CARD_DRAWN`、`CARDS_DRAWN`、または `sourceEventType` が `CARD_DRAWN` / `CARDS_DRAWN` である `REDACTED` draw marker が `viewSequence` 上で連続する区間だけとする。`EFFECT_STARTED`、ダメージ、状態変化、`CARD_DISCARDED`、または `sourceEventType` が `CARD_DISCARDED` である private discard marker が 1 件でも入った時点で区間を閉じ、前後のドローをまたいで合成しない。private discard marker はカードを表示せず、transition の `handCountByPlayer` と `discardCountByPlayer` を同時に正しい before / after 値へ進める独立 entry とする。先行する公開 event がすでに手札減少を反映している場合だけ、marker 自身の手札差分を 0 としてよい。同じ source draw を individual event と batch event の両方で投影してはならず、server projection は batch の `CARDS_DRAWN` か構成する `CARD_DRAWN` 群のどちらか一方だけを出力する。
...
- 致死ダメージでも event の順序を変えない。終局シーケンスは `DAMAGE_DEALT` を 180 ms に短縮し、必要な `BLOCK_REDUCED`（180 ms）、`ENTITY_DAMAGED`（240 ms）、`CARD_DISCARDED`（180 ms）の順で再生する。その直後に 240 ms の terminal barrier を置き、`MATCH_FINISHED` は終局専用の 1,080 ms 表示で再生する。最悪ケースでも `180 + 180 + 240 + 180 + 240 + 1,080 = 2,100 ms` に収め、捨て札を飛ばす演出を消さず、結果画面の前に死亡状態を確実に読める。
