...
async function main() {
  ...
  if (!postgresReady) {
    fail('postgres did not become ready within the timeout. Check `docker compose logs postgres`.');
  }
  if (!mailpitReady) {
    fail('mailpit did not become ready within the timeout. Check `docker compose logs mailpit`.');
  }
